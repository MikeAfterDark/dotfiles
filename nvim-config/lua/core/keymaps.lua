...
vim.keymap.set('n', 's', '<Nop>', { desc = "Disable 's' in normal mode" })

-- QoL:
vim.api.nvim_set_keymap('n', '<leader>date', ':lua InsertDate()<CR>', { noremap = true, silent = true })

function InsertDate()
    local date = os.date '%Y-%m-%d' -- Ensure it returns a string
    if type(date) ~= 'string' then
        return
    end
    vim.api.nvim_put({ '[' .. date .. ']' }, '', true, true) -- Wrap date in brackets
end
